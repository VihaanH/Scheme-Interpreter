...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        formals = signature.rest  # Extract the formal parameters
        validate_formals(formals)  # Check that formals are valid
        
        # Create a LambdaProcedure object and define it in the environment
...
def do_and_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    result = True  # Default to True for an empty list
    while expressions is not nil:
        result = scheme_eval(expressions.first, env)
        if is_scheme_false(result):
            return result  # Return immediately on a false value
...
def do_or_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    result = False  # Default to False for an empty list
    while expressions is not nil:
        result = scheme_eval(expressions.first, env)
        if is_scheme_true(result):
            return result  # Return immediately on a true value
        expressions = expressions.rest
    return result
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    # BEGIN PROBLEM 14
    seen_symbols = set()  # To check for duplicates
    while bindings is not nil:
        binding = bindings.first
        if not scheme_listp(binding) or len(binding) != 2:
            raise SchemeError("Each binding must be a pair (symbol, expression).")

        symbol, expression = binding.first, binding.rest.first
        if not scheme_symbolp(symbol):
            raise SchemeError(f"Invalid symbol in binding: {symbol}")
        if symbol in seen_symbols:
            raise SchemeError(f"Duplicate symbol: {symbol}")
        seen_symbols.add(symbol)

        # Evaluate the expression in the current environment
        value = scheme_eval(expression, env)

        # Add the symbol and its value to the respective lists
